...
class FakeManager:
    ...
    async def load_workflow_input_metadata(self, workflow_id):
        assert workflow_id == "wf-1"
        return {
            "value": '{"raw":"input"}',
    ...
    async def load_step_output_metadata(self, workflow_id):
        assert workflow_id == "wf-1"
        return {
            1: {
                "value": '{"raw":"step"}',
                ...
                "editor": {
                    "mode": "python-value",
                    "value": {"greeting": "Hello world", "name_length": 5},
                },
            }
        }
    ...
    async def stage_edited_fork(
        ...
        *,
        ...
        raw_step_output_overrides=None,
        new_workflow_id=None,
...
def test_ui_and_static_assets_are_served() -> None:
    with create_control_plane_client() as client:
        ...
        assert "StepChange" in ui_response.text
...
def test_http_routes_delegate_to_manager() -> None:
    ...
    with create_control_plane_client().__class__(app) as client:
        ...
        assert workflow_response.json()["raw_workflow_input"] == {
            "value": '{"raw":"input"}',
            "serialization": "py_pickle",
            "editor": {
                "mode": "python-args-kwargs",
        ...
        assert steps_response.json()["raw_step_outputs"] == {
            "1": {
                "value": '{"raw":"step"}',
                "serialization": "py_pickle",
                "editor": {
...
def test_fork_route_uses_local_staged_edit_flow_for_raw_workflow_input_override() -> None:
    app = create_control_plane_app()
    manager = FakeManager()
    app.state.conductor_manager = manager
    ...
    with create_control_plane_client().__class__(app) as client:
        response = client.post(
            ...
            json={
                "workflow_id": "wf-1",
                "start_step": 0,
                "raw_workflow_input_override": '[{"@class":"org.example.WorkflowInput"}]',
            },
        )
...
def test_fork_route_uses_local_staged_edit_flow_for_raw_checkpoint_override() -> None:
    app = create_control_plane_app()
    manager = FakeManager()
    app.state.conductor_manager = manager
    ...
    with create_control_plane_client().__class__(app) as client:
        response = client.post(
            ...
            json={
                "workflow_id": "wf-1",
                "start_step": 2,
                "raw_step_output_overrides": {"1": '{"@class":"org.example.StepOneResult"}'},
            },
        )
...
def test_fork_route_validates_raw_input_override_shape() -> None:
    app = create_control_plane_app()
    app.state.conductor_manager = FakeManager()
    ...
    with create_control_plane_client().__class__(app) as client:
        response = client.post(
            ...
            json={
                "workflow_id": "wf-1",
                "start_step": 0,
                "raw_workflow_input_override": {"bad": True},
            },
        )

        assert response.status_code == 400
        assert response.json()["detail"] == "raw_workflow_input_override must be a string"


def test_fork_route_validates_raw_step_output_override_shape() -> None:
    app = create_control_plane_app()
    app.state.conductor_manager = FakeManager()
    ...
    with create_control_plane_client().__class__(app) as client:
        response = client.post(
            ...
            json={
                "workflow_id": "wf-1",
                "start_step": 2,
                "raw_step_output_overrides": ["bad"],
            },
        )

        assert response.status_code == 400
        assert response.json()["detail"] == "raw_step_output_overrides must be a JSON object keyed by step id"
...
def test_derive_input_override_seed_invalid_returns_none() -> None:
    ...
    # Unparseable input that is not valid JSON, base64, or Python repr.
...
def test_derive_input_override_seed_go_runtime_json_scalar_string() -> None:
    """Go executor pre-deserializes DBOS_JSON and forwards the JSON scalar
    directly over the conductor protocol (e.g. '"bob"' for one string arg)."""
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed({"Input": '"bob"'})
    assert seed == {"args": ["bob"], "kwargs": {}}


def test_derive_input_override_seed_go_runtime_json_scalar_number() -> None:
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed({"Input": "42"})
    assert seed == {"args": [42], "kwargs": {}}


def test_derive_input_override_seed_bare_json_array_positional_args() -> None:
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed({"Input": '["a","b",1]'})
    assert seed == {"args": ["a", "b", 1], "kwargs": {}}


def test_derive_input_override_seed_java_jackson_wrapper_array() -> None:
    from control_plane.routes import _derive_input_override_seed

    # Jackson's WRAPPER_ARRAY polymorphic tag for Object[] with two args.
    seed = _derive_input_override_seed(
        {"Input": '["[Ljava.lang.Object;",["world",42]]'}
    )
    assert seed == {"args": ["world", 42], "kwargs": {}}


def test_derive_input_override_seed_java_jackson_empty_args() -> None:
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed({"Input": '["[Ljava.lang.Object;",[]]'})
    assert seed == {"args": [], "kwargs": {}}


def test_derive_input_override_seed_typescript_superjson() -> None:
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed(
        {"Input": '{"json":["world"],"__dbos_serializer":"superjson"}'}
    )
    assert seed == {"args": ["world"], "kwargs": {}}


def test_derive_input_override_seed_typescript_superjson_multi_arg() -> None:
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed(
        {"Input": '{"json":["a",1,true,null],"__dbos_serializer":"superjson"}'}
    )
    assert seed == {"args": ["a", 1, True, None], "kwargs": {}}


def test_derive_input_override_seed_go_dbos_json() -> None:
    import base64

    from control_plane.routes import _derive_input_override_seed

    encoded = base64.b64encode(b'"world"').decode("ascii")
    seed = _derive_input_override_seed({"Input": encoded})
    assert seed == {"args": ["world"], "kwargs": {}}


def test_derive_input_override_seed_go_dbos_json_object_value() -> None:
    import base64
    ...
    encoded = base64.b64encode(_json.dumps(payload).encode("utf-8")).decode("ascii")
    seed = _derive_input_override_seed({"Input": encoded})
    assert seed == {"args": [payload], "kwargs": {}}


def test_derive_input_override_seed_typescript_js_repr_array() -> None:
    """TS executor forwards inputs as a JS-style single-quoted array string,
    e.g. "[ 'bob' ]". JSON parse fails; ast.literal_eval recovers the list."""
    from control_plane.routes import _derive_input_override_seed

    seed = _derive_input_override_seed({"Input": "[ 'bob' ]"})
    assert seed == {"args": ["bob"], "kwargs": {}}

    seed_multi = _derive_input_override_seed({"Input": "[ 'a', 1, True ]"})
    assert seed_multi == {"args": ["a", 1, True], "kwargs": {}}


def test_static_js_contains_serializer_aware_editor_paths() -> None:
    with create_control_plane_client() as client:
        js_response = client.get("/static/app.js")

    assert js_response.status_code == 200
    assert "function isPythonPickleSerialization" in js_response.text
    assert "function isGoDbosJsonSerialization" in js_response.text
    assert "Go DBOS_JSON payloads are shown decoded as JSON" in js_response.text
    assert "Python pickle payloads are decoded through the Python serializer for editing" in js_response.text
    assert "encodeGoDbosJsonPayload" in js_response.text
    assert "python-args-kwargs" in js_response.text
    assert "python-value" in js_response.text
